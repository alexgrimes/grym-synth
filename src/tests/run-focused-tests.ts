...
const missingFiles = testFiles.filter(
  (file) => !fs.existsSync(resolve(process.cwd(), file))
);
...
console.log(
  "Running focused tests for service layer and context management...\n"
);

// Run Jest with only our specific test files
const result = spawnSync("npx", ["jest", "--verbose", ...testFiles], {
  stdio: "inherit",
  shell: true,
});
...
if (result.status !== 0) {
  ...
  console.log(
    "2. Verify that all required interfaces are properly implemented"
  );
  console.log("3. Ensure mock configurations match the current implementation");
  console.log(
    "4. Look for interface mismatches between tests and implementation"
  );
